...
if (!MONGODB_URI.includes('swaraj')) {
    console.error('❌ Safety check failed: MONGODB_URI does not contain "swaraj". Refusing to run on potentially wrong database.');
...
async function clean() {
    try {
        console.log('🔗 Connecting to MongoDB (swaraj)...');
        ...
        const adminPassword = await bcrypt.hash('Swaraj123', 10);
        const admin = await User.create({
            name: 'System Admin',
            email: 'swaraj@hub102.com',
            password: adminPassword,
            role: 'admin'
        });
        console.log('✅ Admin account created:');
        console.log(`   Email: ${admin.email}`);
        console.log('   Password: Swaraj123');
