...
if (!MONGODB_URI.includes('Royalnest')) {
    console.error('❌ Safety check failed: MONGODB_URI does not contain "Royalnest". Refusing to run on potentially wrong database.');
...
async function clean() {
    try {
        console.log('🔗 Connecting to MongoDB (Royalnest)...');
        ...
        const adminPassword = await bcrypt.hash('Royalnest123', 10);
        const admin = await User.create({
            name: 'System Admin',
            email: 'royalnest@hub102.com',
            password: adminPassword,
            role: 'admin'
        });
        console.log('✅ Admin account created:');
        console.log(`   Email: ${admin.email}`);
        console.log('   Password: Royalnest123');
