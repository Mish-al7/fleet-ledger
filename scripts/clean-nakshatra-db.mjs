...
if (!MONGODB_URI.includes('nakshatra')) {
    console.error('❌ Safety check failed: MONGODB_URI does not contain "nakshatra". Refusing to run on potentially wrong database.');
...
async function clean() {
    try {
        console.log('🔗 Connecting to MongoDB (nakshatra)...');
        ...
        const adminPassword = await bcrypt.hash('Nakshatra123', 10);
        const admin = await User.create({
            name: 'System Admin',
            email: 'nakshatra@hub102.com',
            password: adminPassword,
            role: 'admin'
        });
        console.log('✅ Admin account created:');
        console.log(`   Email: ${admin.email}`);
        console.log('   Password: Nakshatra123');
